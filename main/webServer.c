 ...
 */
char *SERVER_TAG = "ws_echo_server";
...
static esp_err_t home_handler(httpd_req_t *req){
    ...
    if (buf_len > 1) {
        ...
        if (httpd_req_get_hdr_value_str(req, "Host", buf, buf_len) == ESP_OK) {
            ESP_LOGI(SERVER_TAG, "Found header => Host: %s", buf);
        }
        free(buf);
    }
    ...
    if (httpd_req_get_hdr_value_len(req, "Host") == 0) {
        ESP_LOGI(SERVER_TAG, "Request headers lost");
    }
    return ESP_OK;
}
...
static const httpd_uri_t home = {
        ...
        .user_ctx   = "<html> <body> <h1>Hello World!</h1> </body> </html>",
        .is_websocket = false
};
